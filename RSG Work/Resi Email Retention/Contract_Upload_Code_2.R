...
mkt <- mkt[,`:=`(Date.Signed = as.Date(Date.Signed, format = "%m/%d/%Y"))]
dt <- copy(mkt)

dt <- dt[, `:=` (Term = "36")]

dt <- dt[, `:=` (Expiration_Date = ymd(Date.Signed) + years(3))]
...
write.table(UPload_this, paste0("//prodpricapp02/d/Analytics/Resi Email Retention/Contract_Upload_2.csv"),
            sep=",",na="",row.names=FALSE)

save.image("//prodpricapp02/d/Analytics/Resi Email Retention/Contracts_NJ_AR_MN.Rdata")
