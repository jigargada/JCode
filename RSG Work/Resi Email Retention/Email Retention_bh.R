...
setkey(contract_grp,infopro_div_nbr)
nrow(contract_grp)
contract_grp <- fee_flag_rates[contract_grp]
nrow(contract_grp)

setkey(fee_flags,account_number)
setkey(contract_grp,acct_nbr)
nrow(contract_grp)
contract_grp <- fee_flags[contract_grp]
nrow(contract_grp)
...
contract_grp_sub <- contract_grp_sub[!is.na(revenue_per_bill) & revenue_per_bill > 0]
nrow(contract_grp_sub)
...
contract_grp_sub_lifts <- contract_grp_sub[,list(infopro_div_nbr,account_number,lifts_per_week,revenue_annual)]
...
contract_grp_sub_lifts <- contract_grp_sub_lifts[,lapply(.SD,sum),by=sumByCols,.SDcols=c("lifts_per_week","revenue_annual")]
...
resi_cust_info_sub3[,`:=`(FRF_Charge= ((as.numeric(FRF))*base_revenue*(as.numeric(frf_rate)/100)))]
resi_cust_info_sub3[,`:=`(ERF_Charge= ((as.numeric(EVR)*as.numeric(erf_rate)/100)*(base_revenue + as.numeric(erf_on_frf) * FRF_Charge)))]

resi_cust_info_sub3[,`:=`(revenue = FRF_Charge+ERF_Charge )]

resi_cust_info_sub3[,rev_per_lift:=revenue/(lifts_per_week*52)]

resi_cust_info_sub3[,`:=`(total_revenue = base_revenue+FRF_Charge+ERF_Charge )]
...
#                                   "close_date","NRD" ,"tenure_years","tenure_months", "FRF_Charge", "ERF_Charge", "total_revenue", "rev_per_lift"))
