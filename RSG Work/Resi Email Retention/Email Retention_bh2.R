...
print_var <- paste0(print_var,"\n","3. The records with invalid email address from the above pulled table are ",nrow(invalid_rec))
...
print_var <- paste0(print_var,"\n","4. The records from the Whole are excluding Invalid Email addresse:  ",nrow(resi_cust_info))

#For Minnesota

resi_cust_info <- resi_cust_info[infopro_div_nbr %in% c("858","894","899","923", "282", "320", "865")]

print_var <- paste0(print_var,"\n","5. The records with Selected markets from Minesotta, New Jerser and Little Rock are ",nrow(resi_cust_info))
...
print_var <- paste0(print_var,"\n","6. The records with Little Rock Zip Exclusion: ",nrow(resi_cust_info))

################ removing Customers from Bloomington#########
resi_cust_info <- resi_cust_info[!(city %in% c("Bloomington", "BLOOMINGTON") & state =="MN")]

print_var <- paste0(print_var,"\n","7. The records after Bloomington City Exclusion: ",nrow(resi_cust_info))
...
F2notecust <- data.table(read.csv("//prodpricapp02/Analytics/Resi Email Retention/Minesotta_F2Notes.csv", header = TRUE))

setnames (F2notecust, "Company.Number", "sucomp")
setnames (F2notecust, "Account..", "suacct")
setnames (F2notecust, "Site", "susite")


F2notecust <- F2notecust[, list(sucomp, suacct, susite)]
...
#                                  asText=TRUE,windows_auth = TRUE, as.is = TRUE))
# 
# l <- list(F2notecust,F2notecust_v1)
# 
# F2notecust <- rbindlist(l)
...
# F2notecust <- unique(F2notecust, by = "suacct")
...
setnames(F2notecust,c("sucomp","suacct","susite"),c("infopro_div_nbr","account_number","site_nbr"))

print_var <- paste0(print_var,"\n","9. NUmber of F2 notes records from Minesotta- ",nrow(F2notecust))

F2notecust <- unique(F2notecust)

print_var <- paste0(print_var,"\n","10. NUmber of UNIQUE F2 notes records from Minesotta- ",nrow(F2notecust))

setkey(F2notecust, infopro_div_nbr,account_number,site_nbr)
nrow(F2notecust)

resi_cust_info_sub <- F2notecust[resi_cust_info_sub]
nrow(resi_cust_info_sub)

print_var <- paste0(print_var,"\n","11. Number of Records in the parent table remain same - ",nrow(resi_cust_info_sub))
