...
rate_upload_2  <- rate_upload_2[,`:=`(revenue_per_bill= as.character(revenue_per_bill),
...
setkey(rate_upload_2, acct_nbr, site_nbr, cont_grp)
rate_upload_3 <- contract_grp_1[rate_upload_2]
...
rate_upload_3[, ':='(effective_date = as.Date(paste0(substr(effective_date,1,4), "-" ,substr(effective_date,5,6),
                                                     "-" ,substr(effective_date,7,8))),
                     Date.Signed = as.Date(Date.Signed))]

rate_upload_3[, ':='(eff_date = as.Date(ifelse(effective_date>Date.Signed, effective_date, Date.Signed), origin = "1970-01-01")+365)]
rate_upload_3[, ':='(Year_1itemized = ifelse(Group == "A11" | Group == "A12", round(as.numeric(revenue_per_bill) + 
...
contract_uploads = data.table(read.csv("//prodpricapp02/d/Analytics/Resi Email Retention/Contract_Upload.csv", header=TRUE))
...
setkey(rate_upload_3, acct_nbr, site_nbr)
setkey(contract_uploads_1, acct_nbr, site_nbr)
rate_upload_3 <- rate_upload_3[contract_uploads_1]

rate_upload_3[, `:=`(acct_nbr = as.numeric(substr(acct_nbr, 5,11)),
...
rate_upload_3 <- rate_upload_3[!(is.na(revenue_distribution_cd)| is.na(recurring_charge_freq))]

setkey(tax_app_code,  company_number, account_number, site, container_group, charge_code, charge_rate, charge_type, charge_method)
setkey(rate_upload_3, infopro_div_nbr, acct_nbr, site_nbr, container_grp_nbr, charge_code, revenue_per_bill, charge_type, charge_method)
rate_upload_4 <- tax_app_code[rate_upload_3]
...
write.table(upload_2, paste0("//prodpricapp02/d/Analytics/Resi Email Retention/Rate_Upload_Y3_0923.csv"),
